...
def test_option_match():
    ...
    assert Option("-a").match([Option("-x"), Option("-a"), Argument("N")]) == (True, [Option("-x"), Argument("N")], [Option("-a")])
    assert Option("-a").match([Option("-a", value=True), Option("-a")]) == (True, [Option("-a")], [Option("-a", value=True)])
...
def test_argument_match():
    ...
    assert Argument("N").match([Option("-x"), Option("-a"), Argument(None, 5)]) == (True, [Option("-x"), Option("-a")], [Argument("N", 5)])
    assert Argument("N").match([Argument(None, 9), Argument(None, 0)]) == (True, [Argument(None, 0)], [Argument("N", 9)])
...
def test_command_match():
    ...
    assert Command("c").match([Option("-x"), Option("-a"), Argument(None, "c")]) == (True, [Option("-x"), Option("-a")], [Command("c", True)])
    assert Either(Command("add", False), Command("rm", False)).match([Argument(None, "rm")]) == (True, [], [Command("rm", True)])
...
def test_docopt():
    ...
    assert a == {"-v": True, "-q": False, "-r": False, "--help": False, "FILE": "file.py", "INPUT": None, "OUTPUT": None}
...
def test_options_first():
    ...
    assert docopt("usage: prog [--opt] [<args>...]", "this that --opt", options_first=True) == {"--opt": False, "<args>": ["this", "that", "--opt"]}
